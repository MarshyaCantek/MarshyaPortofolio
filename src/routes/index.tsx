...
import { Mail, MapPin, ArrowUpRight, Sparkles, Code2, Palette, Users, Lightbulb, Heart, ExternalLink, Phone } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Portfolio Marshya Salsabilla — Informatics Engineering student passionate about frontend development and modern UI design." },
      { property: "og:title", content: "Marshya Salsabilla — Frontend Developer & UI Designer" },
      { property: "og:description", content: "Crafting modern, responsive, and user friendly digital experiences." },
    ],
  }),
  component: Portfolio,
});
...
const education = [
  { school: "Institut Asia Malang", program: "Informatics Engineering — Undergraduate", period: "2025 — present" },
    ...
    <div className="min-h-screen bg-background text-foreground font-sans">
      ...
      <header className="fixed top-0 inset-x-0 z-50 backdrop-blur-md bg-background/70 border-b border-border/50">
        <nav className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <a href="#home" className="font-display text-2xl tracking-tight">Marshya<span className="text-accent">.</span></a>
          ...
          <a href="#contact" className="inline-flex items-center gap-2 rounded-full bg-primary text-primary-foreground px-5 py-2 text-sm hover:opacity-90 transition">
      ...
      {/* Hero */}
      <section id="home" className="relative pt-36 pb-24 overflow-hidden" style={{ background: "var(--gradient-hero)" }}>
        <img src={heroBg} alt="" width={1536} height={1536} className="absolute inset-0 w-full h-full object-cover opacity-40 mix-blend-multiply pointer-events-none" />
        <div className="relative max-w-6xl mx-auto px-6 grid md:grid-cols-12 gap-10 items-center">
          <div className="md:col-span-8">
            <span className="inline-flex items-center gap-2 rounded-full border border-border bg-card/60 backdrop-blur px-4 py-1.5 text-xs text-muted-foreground">
              <span className="w-2 h-2 rounded-full bg-accent animate-pulse" /> Available for collaborations
            </span>
            <h1 className="mt-6 font-display text-6xl md:text-8xl leading-[0.95]">
              Hi, I'm <em className="italic text-accent">Marshya</em><br />
              <span className="text-foreground/90">a frontend & UI designer</span>
            </h1>
            <p className="mt-8 max-w-xl text-lg text-muted-foreground leading-relaxed">
              Informatics Engineering student crafting modern, responsive, and user-friendly interfaces that blend
              functionality with attractive visual design.
            </p>
            <div className="mt-10 flex flex-wrap gap-4">
              <a href="#projects" className="inline-flex items-center gap-2 rounded-full bg-primary text-primary-foreground px-7 py-3 text-sm font-medium hover:opacity-90 transition">
                View my work <ArrowUpRight className="w-4 h-4" />
              </a>
              <a href="#contact" className="inline-flex items-center gap-2 rounded-full border border-border bg-card/70 backdrop-blur px-7 py-3 text-sm hover:bg-card transition">
                Get in touch
              </a>
            </div>
            <div className="mt-12 flex flex-wrap gap-x-8 gap-y-3 text-sm text-muted-foreground">
              <span className="inline-flex items-center gap-2"><MapPin className="w-4 h-4" /> Malang, Indonesia</span>
              <a href="mailto:marshyasalsabilla@gmail.com" className="inline-flex items-center gap-2 hover:text-foreground transition">
                <Mail className="w-4 h-4" />
                <span>marshyasalsabilla@gmail.com</span>
              </a>
              <a href="https://wa.me/6285856814316" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 hover:text-foreground transition">
          ...
          <div className="md:col-span-4">
            <div className="relative aspect-[3/4] rounded-3xl overflow-hidden" style={{ boxShadow: "var(--shadow-soft)" }}>
              <div className="absolute inset-0" style={{ background: "var(--gradient-warm)" }} />
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="font-display text-[12rem] italic text-foreground/15 leading-none">M</span>
              </div>
              <div className="absolute bottom-6 left-6 right-6 rounded-2xl bg-card/80 backdrop-blur p-4 text-sm" style={{ boxShadow: "var(--shadow-card)" }}>
      ...
      <section id="about" className="py-28 max-w-6xl mx-auto px-6">
        <div className="grid md:grid-cols-12 gap-12">
          ...
          <div className="md:col-span-8 space-y-6 text-lg leading-relaxed text-foreground/80">
            <p>
              I'm an Informatics Engineering student deeply interested in <em className="italic text-accent not-italic">frontend development</em> and design.
              I love turning ideas into clean, intuitive interfaces — the kind that feel effortless to use.
            </p>
            <p>
              My work sits at the intersection of code and craft. I'm currently exploring new technologies,
              sharpening my design skills, and building creative digital experiences that look as good as they work.
            </p>
          </div>
        </div>
      </section>
      ...
      <section id="skills" className="py-24 bg-muted/40 border-y border-border">
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex items-end justify-between flex-wrap gap-4 mb-12">
            <div>
              <div className="text-xs uppercase tracking-widest text-muted-foreground">02 — Skills</div>
          ...
          <div className="grid md:grid-cols-2 gap-10">
            <div>
              <h3 className="text-sm font-medium uppercase tracking-wider text-muted-foreground mb-5">Hard Skills</h3>
              <div className="flex flex-wrap gap-3">
                {hardSkills.map((s) => (
                  <span key={s} className="rounded-full bg-card border border-border px-5 py-2.5 text-sm hover:bg-secondary transition">
                    {s}
                  </span>
                ))}
              </div>
            ...
            <div>
              <h3 className="text-sm font-medium uppercase tracking-wider text-muted-foreground mb-5">Soft Skills</h3>
              <div className="grid grid-cols-2 gap-3">
                {softSkills.map(({ icon: Icon, label }) => (
                  <div key={label} className="flex items-center gap-3 rounded-2xl bg-card border border-border px-4 py-3">
                    <span className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: "var(--gradient-warm)" }}>
      ...
      <section id="projects" className="py-28 max-w-6xl mx-auto px-6">
        <div className="mb-14">
          <div className="text-xs uppercase tracking-widest text-muted-foreground">03 — Selected Work</div>
          <h2 className="mt-3 font-display text-5xl md:text-6xl">Recent projects.</h2>
        </div>
        <div className="grid md:grid-cols-2 gap-8">
          {projects.map((p) => (
            <article key={p.title} className="group rounded-3xl overflow-hidden bg-card border border-border hover:border-accent/50 transition" style={{ boxShadow: "var(--shadow-card)" }}>
              <div className="aspect-[4/3] overflow-hidden">
                <img src={p.img} alt={p.title} width={1024} height={1024} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition duration-700" />
              </div>
              <div className="p-7">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <ArrowUpRight className="w-5 h-5 text-muted-foreground group-hover:text-accent group-hover:-translate-y-1 group-hover:translate-x-1 transition" />
                </div>
      ...
      <section id="certificates" className="py-28 max-w-6xl mx-auto px-6">
        <div className="mb-14 flex items-end justify-between flex-wrap gap-4">
          <div>
            <div className="text-xs uppercase tracking-widest text-muted-foreground">03 — Certificates</div>
      ...
      <section id="experience" className="py-28 bg-muted/40 border-y border-border">
        <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-16">
          <div>
            <div className="text-xs uppercase tracking-widest text-muted-foreground">04 — Experience</div>
          ...
          <div>
            <div className="text-xs uppercase tracking-widest text-muted-foreground">05 — Education</div>
              ...
              {education.map((ed) => (
                <div key={ed.school} className="rounded-2xl bg-card border border-border p-5">
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <div className="text-xs text-muted-foreground whitespace-nowrap">{ed.period}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section id="contact" className="py-32">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <div className="text-xs uppercase tracking-widest text-muted-foreground">06 — Contact</div>
          ...
          <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg mx-auto">
            ...
            >
              <span className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "var(--gradient-warm)" }}>
            ...
            >
              <span className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "var(--gradient-warm)" }}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 text-foreground/70"><rect width="20" height="20" x="2" y="2" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>
              </span>
              <div>
            ...
            >
              <span className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "var(--gradient-warm)" }}>
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-foreground/70"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
              </span>
              <div>
